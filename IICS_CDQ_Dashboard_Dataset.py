...
# History data capture duration. Example - 10 means capture profiling execution data for last 10 executions of a profile.
history_rec_count = 10

# If Folder doesn't exist then create
if not os.path.exists(csv_file_path):
    print('Folder does not exist. Creating the folder - ' + csv_file_path)
    os.makedirs(csv_file_path)
    # Change working directory to create CSV files
    os.chdir(csv_file_path)
else:
    # Change working directory to create CSV files
    print('Folder ' + csv_file_path + 'already exist')
    os.chdir(csv_file_path)
...
def get_latest_run_key_and_metadata(in_profile_id):
    ...
    print('API to get get Profile latest run key + Rule tagged to a profile, Project, Folder information executed - '+ profile_api_url_1)
...
def get_column_profiling_result(in_profile_id, in_profile_name, in_run_key, in_i, in_file_name, in_seq):
    ...
    print('API to get column profiling result executed - ' + column_profiling_api_url)
...
def get_top_n_value_frequency(in_Record_Type, in_profile_id, in_profile_name, in_column_id, in_column_name, in_runkey,in_file_name, in_i):
    ...
    print('API to get Top N value frequencies by column id executed - ' + value_frequency_api_url)
...
def get_profile_execution_stats(in_profile_id, in_profile_name, in_run_key, in_i, in_file_name, in_Record_Type, in_Loop_1):
    in_run_key = str(in_run_key)
    profile_execution_stats_api_url = profile_execution_api_url + in_profile_id + "/filter?sortBy=runKey&sortOrder=DESC"
    headers = {
        "Accept": "application/json",
        'content-type': 'application/json',
        "IDS-SESSION-ID": session_id
    ...
    payload = json.dumps({
        "exceptLastNRun": -1,
        "purgeQueryType": "EXCEPT_LAST_N_RUNS"
    })
    response = requests.request("POST", profile_execution_stats_api_url, headers=headers, data=payload).json()['items']
    print('API to to get profile execution stats executed - ' + profile_execution_stats_api_url)
    try:
        profile_start_time = datetime.datetime.fromtimestamp(int(response[in_Loop_1]['startTime']) / 1000).strftime("%Y-%m-%d %H:%M:%S")
        profile_end_time = datetime.datetime.fromtimestamp(int(response[in_Loop_1]['endTime']) / 1000).strftime("%Y-%m-%d %H:%M:%S")
        profile_execution_Time = response[in_Loop_1]['executionTime']
        rows_Processed = response[in_Loop_1]['rowsProcessed']
        run_status = response[in_Loop_1]['status']
        runCostInMB = response[in_Loop_1]['runCostInMB']
        difference = (datetime.datetime.fromtimestamp(int(response[in_Loop_1]['endTime']) / 1000) - datetime.datetime.fromtimestamp(int(response[in_Loop_1]['startTime']) / 1000)).total_seconds()
        array_dict = {
        ...
        logging.exception('Exception captured in function get_profile_execution_stats() while processing profile - ' +in_profile_id+' - '+in_profile_name+'. Please check that profile execution (Run Key='+in_run_key+') data is available.'+ str(e1))
...
print('Getting list of profiles by calling function get_profile_list()')
for profile_id_out in get_profile_list():

    # Capture Latest run key with Metadata details for each profile in a CSV file
    print('Capturing Latest run key with Metadata details for each profile in a CSV file by calling function get_latest_run_key_and_metadata()')
    try:
        latest_run_key = get_latest_run_key_and_metadata(profile_id_out['id'])
        # i_runkey_metadata += 1
        # i_runkey_outfield += 1
    except Exception as e2:
        logging.exception('Exception captured while calling function get_latest_run_key_and_metadata() while processing profile ' +profile_id_out['id']+' - '+profile_id_out['name']+'. Please check that this profile is executed at least once and user is able to access the profile.' + str(e2))
        continue
    # Capture Colume Profiling Result Data for each profile in a CSV file - Upto Last 10 runs
    print('Capturing Column Profiling Result Data for each profile in a CSV file - Upto ' + str(history_rec_count) + ' runs')
    try:
        for Loop_1 in range(min(history_rec_count, latest_run_key)):
            ...
            get_profile_execution_stats(profile_id_out['id'], profile_id_out['name'], profile_run_key, i_p_stats_result,Profile_execution_stats, Record_Type, Loop_1)
            try:
                ...
                logging.exception('Exception captured while calling function get_column_profiling_result() while processing profile '+profile_id_out['id']+' - '+profile_id_out['name']+'. '+ str(e3))
                continue
    except Exception as e4:
        logging.exception('Exception captured while running Loop_1 while processing profile '+profile_id_out['id']+' - '+profile_id_out['name']+'. '+ str(e4))
        continue

# Capture Top-N Value Frequency Result Data for each profile and only mapplet columns in a CSV file - Upto previous 9 runs
print('Capturing Top-N Value Frequency Result Data for each profile and only mapplet columns in a CSV file Upto ' + str(history_rec_count) + ' runs')
